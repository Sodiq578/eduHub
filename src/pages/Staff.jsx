...
import { 
  ...
  HiOutlineViewGrid, HiOutlineViewList, HiOutlineViewBoards
} from 'react-icons/hi';
...
const Staff = () => {
  ...
  const loadStaff = () => {
    ...
    if (stored) {
      ...
      const defaultStaff = [
        ...
        { id: 4, name: 'Nodira Akbarova', position: 'Kutubxonachi', department: 'Kutubxona', phone: '+998 94 444 5566', email: 'nodira@edumanage.com', hireDate: '2022-01-10', salary: 2200000, address: 'Toshkent sh.', status: 'active', avatar: 'N' },
        { id: 5, name: 'Shavkat Toshpulatov', position: 'Texnik xodim', department: 'Texnik xodim', phone: '+998 95 555 6677', email: 'shavkat@edumanage.com', hireDate: '2023-03-01', salary: 1800000, address: 'Toshkent sh.', status: 'active', avatar: 'S' },
      ];
  ...
  const getDepartmentColor = (dept) => {
    const colors = {
      ...
      'Xavfsizlik': '#ef4444',
      'Texnik xodim': '#64748b',
      'Tibbiyot xodimi': '#ec489a'
    };
    return colors[dept] || '#64748b';
  };

  const renderStaffCard = (s) => {
    const deptColor = getDepartmentColor(s.department);

    if (viewMode === 'grid') {
      return (
        <div key={s.id} className="staff-card grid-card">
          <div className="staff-avatar" style={{ background: deptColor }}>{s.avatar}</div>
          <div className="staff-info">
            <h3>{s.name}</h3>
            <p className="staff-position">{s.position}</p>
            <p className="staff-dept" style={{ color: deptColor }}>{s.department}</p>
          ...
          {isAdmin && (
            <div className="staff-actions">
              <button className="edit-btn" onClick={() => handleEdit(s)}><HiOutlinePencil /> Tahrirlash</button>
              <button className="delete-btn" onClick={() => handleDelete(s.id)}><HiOutlineTrash /> O'chirish</button>
            </div>
          )}
        </div>
      );
    ...
    if (viewMode === 'list') {
      return (
        <div key={s.id} className="staff-card list-card">
          <div className="list-avatar" style={{ background: deptColor }}>{s.avatar}</div>
          <div className="list-info">
            <div className="list-header">
              <h3>{s.name}</h3>
              <span className="dept-badge" style={{ background: `${deptColor}20`, color: deptColor }}>{s.department}</span>
            </div>
            <p className="list-position">{s.position}</p>
            <div className="list-details">
              <span><HiOutlinePhone /> {s.phone}</span>
              <span><HiOutlineMail /> {s.email}</span>
              <span><HiOutlineCalendar /> {s.hireDate}</span>
              <span>💰 {s.salary?.toLocaleString()} so'm</span>
            </div>
          </div>
          {isAdmin && (
    ...
    // Compact mode
    return (
      <div key={s.id} className="staff-card compact-card">
        <div className="compact-avatar" style={{ background: deptColor }}>{s.avatar}</div>
        <div className="compact-info">
          <h4>{s.name}</h4>
          <p>{s.position}</p>
        </div>
        <div className="compact-dept" style={{ background: `${deptColor}20`, color: deptColor }}>{s.department}</div>
        {isAdmin && (
          <div className="compact-actions">
            <button className="compact-edit" onClick={() => handleEdit(s)}><HiOutlinePencil /></button>
            <button className="compact-delete" onClick={() => handleDelete(s.id)}><HiOutlineTrash /></button>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="staff-page">
      <div className="page-header">
        <div>
          <h1>Xodimlar</h1>
          <p>Jami {totalStaff} nafar xodim | {activeStaff} ta aktiv | Oylik fond: {totalSalary.toLocaleString()} so'm</p>
        </div>
      ...
      <div className="staff-stats">
        ...
        <div className="stat-card"><div className="stat-value">{staff.filter(s => s.department === 'Xavfsizlik').length}</div><div className="stat-label">Xavfsizlik</div></div>
        <div className="stat-card"><div className="stat-value">{staff.filter(s => s.department === 'Kutubxona').length}</div><div className="stat-label">Kutubxona</div></div>
      </div>

      <div className="filters-bar">
        <div className="search-wrapper">
          <HiOutlineSearch />
          <input type="text" placeholder="Ism yoki lavozim bo'yicha qidirish..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      ...
      <div className={`staff-container ${viewMode}`}>
        {filteredStaff.length === 0 ? (
          <div className="empty-state">
            <HiOutlineUser size={48} />
            <p>Hech qanday xodim topilmadi</p>
            {isAdmin && <button className="btn-primary" onClick={handleAdd}>Yangi xodim qo'shish</button>}
          </div>
        ) : (
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{editingStaff?.id ? 'Xodim tahrirlash' : 'Yangi xodim'}</h2>
            ...
            <div className="modal-body">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  <label>Telefon</label>
                  <input type="tel" value={editingStaff?.phone || ''} onChange={(e) => setEditingStaff({ ...editingStaff, phone: e.target.value })} />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input type="date" value={editingStaff?.hireDate || ''} onChange={(e) => setEditingStaff({ ...editingStaff, hireDate: e.target.value })} />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label>Maosh (so'm)</label>
                  <input type="number" value={editingStaff?.salary || 0} onChange={(e) => setEditingStaff({ ...editingStaff, salary: parseInt(e.target.value) })} />
                </div>
                <div className="form-group">
                  <label>Manzil</label>
                  <input type="text" value={editingStaff?.address || ''} onChange={(e) => setEditingStaff({ ...editingStaff, address: e.target.value })} />
                </div>
              </div>
            </div>
